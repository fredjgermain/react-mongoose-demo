...
export function Input({...props}:IInput) { 
  ...
  const width = props.sizeFunc ? 
    ...
    {width: `${SetSize(value)}ch`}; 

  // Calculate input width concatenate width defined above with inputAttribute.style if any other style has been defined. 
  const style = {...props.inputAttribute?.style, ...width} 
  
  // Regroups to arguments to pass to input tag
  const inputArgs = {...props.inputAttribute, type, value, placeholder, onChange, onKeyUp, ...{style}} 

  if(type==='checkbox') 
    return <input {...inputArgs} {...{checked:value} } /> 
